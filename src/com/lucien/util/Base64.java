...
public final class Base64 {
	...
	static private final boolean fDebug = true;
	...
	public static String encode(byte[] binaryData) {
		...
		int i = 0;
		if (fDebug) {
			MyUtil.debug("number of triplets = " + numberTriplets);
		}
		...
		for (int line = 0; line < numberLines - 1; line++) {
			for (int quartet = 0; quartet < 19; quartet++) {
				...
				if (fDebug) {
					MyUtil.debug("b1= " + b1, ", b2= " + b2, ", b3= " + b3);
				}
				...
				if (fDebug) {
					MyUtil.debug("val2 = " + val2);
					MyUtil.debug("k4 = " + (k << 4));
					MyUtil.debug("vak = " + (val2 | (k << 4)));
				}
	...
	public static void main(String[] args) {
		String encode = encode("中华人民共和国".getBytes());
		MyUtil.debug(encode);
